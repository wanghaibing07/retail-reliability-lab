...
> 以上是该批次合并时的历史状态。2026-09-24 已部署 Alertmanager，独立测试告警的触发及恢复邮件均已验收；`RetailUIProbeFailed` 的指标驱动触发与恢复邮件也已在一次[受控判据演练](stage4-metric-driven-alert-drill.md)中验收。当前规则已增加时序行为测试与 [排障 Runbook](../runbooks/retail-ui-probe-alert.md)。
